...
def plotfigs2(xx, selectedy, fileprefix=None, printselectionnumbers = False):
    ...
    while counter < int(totm/perpic_m) + 1:
        ...
        gs = gridspec.GridSpec(int(int(perpic_m/perrow)+offset/perrow), int(perrow), wspace=0.0, hspace=0.0)
        fig = plt.figure()
        ...
        for i in np.array(range(begin_at + offset, endd)):
            ...
            ax.imshow(np.array(xx[i - offset, :]).reshape(32, 32, 3))
            ax.axis('off')
...
def run(filename,  gamma, m, k, ktype, outfig, critoutfig,testfile):
    ...
    if k > 0:
        ...
        if critoutfig is not None:
            plotfigs2(digitsdat.X[critselected[critsortedindx], :], critselectedy[critsortedindx], critoutfig)
...
def main(
    ...
    if do_output_pics == 1:
        outfig = os.path.join(output_prefix, 'protos')
        critoutfig = os.path.join(output_prefix, 'crit')
...
if __name__ == '__main__':
    ...
    m = 100  # total number of prototypes to select
    alltestm = np.array([10,30,50,70,80,90,100])
    #alltestm = np.array(
